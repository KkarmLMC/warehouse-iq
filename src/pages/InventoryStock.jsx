    ...
    <div className="page-content fade-in">
      ...
      <div style={{ display: 'flex', gap: 'var(--sp-2)', marginBottom: 'var(--sp-4)', overflowX: 'auto', paddingBottom: 2, scrollbarWidth: 'none', flexWrap: 'wrap' }}>
        <button onClick={() => setSelectedWarehouse(null)}
          style={{ flexShrink: 0, padding: 'var(--sp-1) var(--sp-3)', borderRadius: 'var(--r-full)', border: `1px solid ${!selectedWarehouse ? 'var(--navy)' : 'var(--border-l)'}`, background: !selectedWarehouse ? 'var(--navy)' : 'transparent', color: !selectedWarehouse ? '#fff' : 'var(--text-2)', fontSize: 'var(--fs-xs)', fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>
          All Warehouses
        </button>
        {warehouses.map(w => (
          <button key={w.id} onClick={() => setSelectedWarehouse(w.id)}
            style={{ flexShrink: 0, padding: 'var(--sp-1) var(--sp-3)', borderRadius: 'var(--r-full)', border: `1px solid ${selectedWarehouse === w.id ? 'var(--navy)' : 'var(--border-l)'}`, background: selectedWarehouse === w.id ? 'var(--navy)' : 'transparent', color: selectedWarehouse === w.id ? '#fff' : 'var(--text-2)', fontSize: 'var(--fs-xs)', fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>
        ...
        {[['all', 'All'], ['low', 'Low Stock'], ['out', 'Out of Stock']].map(([val, lbl]) => (
          <button key={val} onClick={() => setStockFilter(val)}
            style={{ flexShrink: 0, padding: 'var(--sp-1) var(--sp-3)', borderRadius: 'var(--r-full)', border: `1px solid ${stockFilter === val ? 'var(--red)' : 'var(--border-l)'}`, background: stockFilter === val ? 'var(--red)' : 'transparent', color: stockFilter === val ? '#fff' : 'var(--text-2)', fontSize: 'var(--fs-xs)', fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>
            {lbl}
          </button>
        ))}
      </div>
